    ...
    <AdminLayout>
        <section class="space-y-8">
            ...
            <div class="border-b border-[#00f5a0]/30 pb-4">
                <h1 class="font-orbitron text-3xl font-bold tracking-wide text-white">Users</h1>
                <p class="text-xl text-white">Manage registered users and their roles.</p>
            </div>

            <!-- Table with search toolbar -->
            <BaseTable :title="'Users'" :columns="columns" :data="users" :pagination="pagination" @page-changed="fetchUsers">
                <template #toolbar>
                    <input
                        v-model="search"
                        type="text"
            ...
            >
                ...
                <template #default="{ field, modelValue, update }">
                    <div v-if="field.key === 'role'">
                        ...
                        <button class="mt-1 rounded bg-blue-500 px-3 py-1 text-white" @click="update(modelValue === 'admin' ? 'user' : 'admin')">
                            {{ modelValue }}
                        </button>
                    </div>
                </template>
...
async function updateUser(updatedData) {
    try {
        const response = await axios.post(route('admin.users.update'), updatedData);
...
async function deleteUser(user) {
    try {
        ...
        const response = await axios.post(route('admin.users.destroy'), { user_id: user.user_id });
